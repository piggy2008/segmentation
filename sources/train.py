...
def train_model(model, num_classes, dataloaders, criterion, optimizer, device, dest_dir, num_epochs=25, save_epoch=25, model_name='resnet101'):
    since = time.time()

    val_acc_history = []
    logfile = open(os.path.join(dest_dir, 'log.txt'), 'w')
    ...
    for epoch in range(1, num_epochs+1):
        ...
        print('-' * 10)
        logfile.write('Epoch {}/{}'.format(epoch, num_epochs) + '\n')
        logfile.write('-' * 10 + '\n')
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    if 'resnet' in model_name:
                        ...
                        outputs2, outputs, _ = model(inputs)
                        h, w = labels.size(1), labels.size(2)
                        outputs = F.interpolate(outputs, size=(h, w), mode='bilinear', align_corners=True)
                        outputs2 = F.interpolate(outputs2, size=(h, w), mode='bilinear', align_corners=True)
                        loss = (criterion(outputs, labels) + criterion(outputs2, labels)) / 2
            ...
            logfile.write('{} Loss: {:.4f} Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc) + '\n')
            ...
            if 0 == epoch%save_epoch:
                current_model_path = os.path.join(dest_dir, f"checkpoint_{epoch:04}_Skydiver.pth")
    ...
    logfile.write('Training complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60) + '\n')
